#ifndef DEMOSTATS_HL2DEMO_STRING_TABLE_UPDATE_H
#define DEMOSTATS_HL2DEMO_STRING_TABLE_UPDATE_H
...
namespace hl2demo {
    ...
    class string_table_update_t : dstruct {
    public:
        string_table_update_t(uint32_t entries, uint32_t max_entries, uint32_t user_data_size,
                ...
                m_entry_bits(0) {
            while (max_entries >>= 1u) ++m_entry_bits;
        }

        void read() override;

        virtual void printDebug(uint32_t indent) const override {};
...
#endif //DEMOSTATS_HL2DEMO_STRING_TABLE_UPDATE_H
